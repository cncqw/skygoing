...
Page({
        ...
        wx.chooseAddress({
            success: function (res) {
                ...
                e.setData({
                    ...
                    // province_id: res.provinceName,
                    // city_id: res.cityName,
                    // county_id: res.countyName,
    ...
    saveData: function(i) {
        var p,c,q,t=this;
        var h = wx.getStorageSync("region");
        for (var u = 0; u <h.length;u++){
            if (t.data.province.indexOf(h[u].region_name)>-1){
                p = h[u].region_id;
                //console.log('p', p)
                break;
            }
        ...
        for (var j = 0; j < h.length; j++) {
            if (t.data.city.indexOf(h[j].region_name) > -1) {
                c = h[j].region_id;
                //console.log('c',c)
                break;
            }
        ...
        for (var y = 0; y < h.length; y++) {
            if (t.data.county.indexOf(h[y].region_name) > -1) {
                q = h[y].region_id;
                //console.log('q', q)
                break;
            }
        ...
        var  e = i.detail.value, a = wx.getStorageSync("token"), n = {
            consignee: e.consignee,
            province: p,//t.data.province_id,
            city:c,// t.data.city_id,
            district:q,// t.data.county_id,
            mobile: e.mobile,
            address: e.address
        };
        wx.request({
                ...
                if (500 == a) {
                    ...
                    success: function() {
                      ...
                      if (getCurrentPages().length > 3) {
                        var _page = getCurrentPages()[getCurrentPages().length - 3];
                        console.log(_page);
                        if (_page.route == 'pages/goods/index' || _page.route == 'pages/group/goods') {
                          g.globalData._linkTo = '/pages/flow/checkout'
                          wx.navigateBack({
                            delta: 2
                          })
                          return;
